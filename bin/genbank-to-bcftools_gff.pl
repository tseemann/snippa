...
for my $seq (@seq) {
  for my $f ($seq->get_SeqFeatures) {
    ...
    my $biotype = $f->primary_tag eq 'CDS' ? 'protein_coding' : $f->primary_tag;
    $counter++;
    ...
    $f->add_tag_value('ID', $f->primary_tag.':'.$id);
    $f->add_tag_value('Parent', "transcript:$id");
    $f->frame(0);
    $f->add_tag_value('biotype', $biotype);
    
    my $mrna = Bio::SeqFeature::Generic->new(
      -seq_id  => $f->seq_id,
      -start   => $f->start,
      -end     => $f->end,
      -strand  => $f->strand,
    ...
    print $mrna->gff_string($gff_factory),"\n";

    $mrna->remove_tag('ID');
    $mrna->add_tag_value('ID', "transcript:$id");
    $mrna->add_tag_value('Parent', "gene:$id");
